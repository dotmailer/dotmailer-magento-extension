    ...
    class RaygunClient
    {
        ...
        */
        public function __construct($key, $useAsyncSending = true,
            $debugSending = false, $disableUserTracking = false
        ) {
            $this->apiKey          = $key;
        ...
        */
        public function SendError($errno, $errstr, $errfile, $errline,
            $tags = null, $userCustomData = null, $timestamp = null
        ) {
        ...
        * @return The HTTP status code of the result when transmitting the message to Raygun.io
        */
        public function SendException($exception, $tags = null,
            $userCustomData = null, $timestamp = null
        ) {
        ...
        *
        */
        public function SetUser($user = null, $firstName = null,
            $fullName = null, $email = null, $isAnonymous = null, $uuid = null
        ) {
            $timestamp = time() + 60 * 60 * 24 * 30;

            $this->firstName = $this->StoreOrRetrieveUserCookie(
                'rgfirstname', $firstName
            );
            $this->fullName  = $this->StoreOrRetrieveUserCookie(
                'rgfullname', $fullName
            );
            $this->email     = $this->StoreOrRetrieveUserCookie(
                'rgemail', $email
            );

            $this->uuid        = $this->StoreOrRetrieveUserCookie(
                'rguuidvalue', $uuid
            );
            $this->isAnonymous = $this->StoreOrRetrieveUserCookie(
                'rgisanonymous', $isAnonymous ? 'true' : 'false'
            ) == 'true' ? true : false;
        ...
        private function StoreOrRetrieveUserCookie($key, $value)
        {
            ...
            if (is_string($value)) {
                ...
                if (array_key_exists($key, $_COOKIE)) {
                    if ($_COOKIE[$key] != $value && php_sapi_name() != 'cli'
                        && ! headers_sent()
                    ) {
                        setcookie($key, $value, $timestamp);
                    }
        ...
        */
        private function BuildMessage($errorException, $timestamp = null)
        {
            ...
            if ($this->user != null) {
                $message->Details->User = new RaygunIdentifier(
                    $this->user, $this->firstName, $this->fullName,
                    $this->email, $this->isAnonymous, $this->uuid
                );
        ...
        private function AddTags(&$message, $tags)
        {
            if (is_array($tags)) {
                ...
                throw new \Raygun4php\Raygun4PhpException(
        ...
        private function AddUserCustomData(&$message, $userCustomData)
        {
            if ($this->is_assoc($userCustomData)) {
                ...
                throw new \Raygun4php\Raygun4PhpException(
                    "UserCustomData must be an associative array"
                );
            }
        }
        ...
        public function Send($message)
        {
            if (empty($this->apiKey)) {
                throw new \Raygun4php\Raygun4PhpException(
                    "API not valid, cannot send message to Raygun"
                );
            }
        ...
        {
            $headers = 0;
            $remote  = $this->transport . '://' . $this->host . ':'
                . $this->port;

            $context = stream_context_create();
            $result  = stream_context_set_option(
                $context, 'ssl', 'verify_host', true
            );

            if ( ! empty($cert_path)) {
                $result = stream_context_set_option(
                    $context, 'ssl', 'cafile', $cert_path
                );
                $result = stream_context_set_option(
            ...
            if ($this->useAsyncSending
                ...
                $fp = stream_socket_client(
                    $remote, $err, $errstr, 10, STREAM_CLIENT_CONNECT, $context
                );
                ...
                if ($fp) {
                    ...
                    $errMsg = "<br/><br/>"
                        . "<strong>Raygun Warning:</strong> Couldn't send asynchronously. ";
                    $errMsg .= "Try calling new RaygunClient('apikey', FALSE); to use an alternate sending method, or RaygunClient('key', FALSE, TRUE) to echo the HTTP response"
                        . "<br/><br/>";
                    echo $errMsg;
        ...
        function toJsonRemoveUnicodeSequences($struct)
        {
            return preg_replace_callback(
                "/\\\\u([a-f0-9]{4})/", function ($matches) {
                return iconv(
                    'UCS-4LE', 'UTF-8', pack('V', hexdec("U$matches[1]"))
                );
            }, json_encode($struct)
            );
        }
        ...
        function filterParamsFromMessage($message, $replace = '[filtered]')
        {
            ...
            foreach ($filterParams as $filterKey => $filterFn) {
                if ( ! is_callable($filterFn)) {
                    $filterParams[$filterKey] = function ($key, $val) use (
                        $replace
                    ) {
                        return $replace;
                    };
            ...
            if ($message->Details->Request->Headers) {
                array_walk_recursive(
                    $message->Details->Request->Headers, $walkFn
                );
            }
            ...
            if ($message->Details->UserCustomData) {
                array_walk_recursive(
                    $message->Details->UserCustomData, $walkFn
                );
            }
