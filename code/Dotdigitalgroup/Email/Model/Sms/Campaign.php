...
class Dotdigitalgroup_Email_Model_Sms_Campaign
{
     ...
     */
    protected $_vars
        = array('/customer_name/', '/order_number/', '/{{var /', '/}}/');
    ...
    public function sendSms()
    {
        ...
        foreach ($this->_allsms as $num) {
            ...
            $enabled = (bool)Mage::getStoreConfig(
                constant(
                    'Dotdigitalgroup_Email_Helper_Config::XML_PATH_CONNECTOR_SMS_ENABLED_'
                    . $num
                )
            );
            if ($enabled) {
                $status  = Mage::getStoreConfig(
                    constant(
                        'Dotdigitalgroup_Email_Helper_Config::XML_PATH_CONNECTOR_SMS_STATUS_'
                        . $num
                    )
                );
                $message = $this->_processMessage(
                    Mage::getStoreConfig(
                        constant(
                            'Dotdigitalgroup_Email_Helper_Config::XML_PATH_CONNECTOR_SMS_MESSAGE_'
                            . $num
                        )
                    )
                );
                $match   = preg_match(
                    self::UK_TELEPHONE_PATTERN, $this->_customerTelephone
                );
                if ($match != false) {
                    $codePhone = preg_replace(
                        '/\A(0){1}+/', '+44', $this->_customerTelephone
                    );
                    //status and telephone valid
                    if ($this->_status == $status) {
                        Mage::helper('ddg')->log(
                            'sending sms message with status : ' . $status
                            . ' and ' . $codePhone
                        );
                    ...
                    Mage::helper('ddg')->log(
                        'SMS: phone not valid for UK : '
                        . $this->_customerTelephone
                    );
                }
            }
        }
