...
class Dotdigitalgroup_Email_Model_Newsletter_Observer
{
    ...
    ) 
    {
        ...
        $websiteId        = Mage::app()->getStore($subscriber->getStoreId())
            ->getWebsiteId();
        ...
        try {
            ...
            Mage::logException($e);
            Mage::helper('ddg')->getRaygunClient()->SendException(
                $e, array(Mage::getBaseUrl('web'))
            );
        }
    ...
    ) 
    {
        ...
        $programId = Mage::helper('ddg')->getAutomationIdByType(
            'XML_PATH_CONNECTOR_AUTOMATION_STUDIO_SUBSCRIBER', $websiteId
        );
        //not mapped ignore
        if ( ! $programId) {
            return;
        ...
        try {
            ...
            $enrolmentCollection = Mage::getModel('ddg_automation/automation')
                ->getCollection()
