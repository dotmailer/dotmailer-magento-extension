    ...
    extends Dotdigitalgroup_Email_Model_Abstract_Rest
{
        ...
        if (! $apiEndpoint) {
            ...
            if (is_object($accountInfo) && ! isset($accountInfo->message)) {
                ...
                foreach ($accountInfo->properties as $property) {
                    if ($property->name == 'ApiEndpoint' && $property->value !== '') {
    ...
    {
        $url = $this->apiEndpoint . self::REST_ADDRESS_BOOKS . $addressBookId . '/contacts';
        $this->setUrl($url)
            ->setVerb("POST")
    ...
    {
        $url = $this->apiEndpoint . self::REST_ADDRESS_BOOKS . $addressBookId . '/contacts/' . $contactId;
        $this->setUrl($url)
            ->setVerb('DELETE');
    ...
    {
        $url = $this->apiEndpoint . self::REST_CONTACTS_IMPORT . $importId . "/report";
        $this->setUrl($url)
            ->setVerb('GET');
        $response = $this->execute();
        //log error
    ...
    {
        $url = $this->apiEndpoint . self::REST_DATA_FILEDS . '/' . $name;
        $this->setUrl($url)
            ->setVerb('DELETE');
    ...
    {
        $url = $this->apiEndpoint . self::REST_TRANSACTIONAL_DATA . $name . '/' . $key;
        $this->setUrl($url)
            ->setVerb('GET');
    ...
    {
        $url = $this->apiEndpoint . '/v2/contacts/' . $contactId . '/address-books';
        $this->setUrl($url)
            ->setVerb('GET');
        $response = $this->execute();
        //log error
