...
class Dotdigitalgroup_Email_Helper_Trial extends Mage_Core_Helper_Abstract {
	...
	{
		$formUrl = $this->getMagentoSignupUrl();
		...
		$callback = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB, true)
		            . 'connector/trial/accountcallback';
		...
		$params = array(
			...
			'ip' => $ipAddress,
			'createOnRegion' => true,
			'code' => $this->generateTrialPasscode()
		);
	...
	private function getMagentoSignupUrl()
	{
		...
		return ($urlFromDatabase) ?
			$urlFromDatabase :
			self::API_CONNECTOR_TRIAL_FORM_URL;
	}
	...
	private function generateTrialPasscode()
	{
		...
		Mage::getModel('core/config')->saveConfig(
			self::XML_PATH_CONNECTOR_API_TRIAL_TEMPORARY_PASSCODE,
			$code
		);

		$expiryDate = new Zend_Date();
		$expiryDateString = $expiryDate->add(30, Zend_Date::MINUTE)->toString(Zend_Date::ISO_8601);

		Mage::getModel('core/config')->saveConfig(
			self::XML_PATH_CONNECTOR_API_TRIAL_TEMPORARY_PASSCODE_EXPIRY,
			$expiryDateString
		);
	 ...
	 * @return bool
	 */
	public function authenticateTrialPasscode($trialPasscode)
	{
		$now = new Zend_Date();
		$expiryDateString = Mage::getStoreConfig(self::XML_PATH_CONNECTOR_API_TRIAL_TEMPORARY_PASSCODE_EXPIRY);
		$expiryDate = new Zend_Date($expiryDateString, Zend_Date::ISO_8601);

		if ($now >= $expiryDate)
			return false;

		return $trialPasscode == Mage::getStoreConfig(
				self::XML_PATH_CONNECTOR_API_TRIAL_TEMPORARY_PASSCODE
			);
	}
	 ...
	 * @throws Varien_Exception
	 */
	public function clearTrialPasscode()
	{
